...
const findOneById = async (repaymentId, options = {}) => {
  try {
    const result = await GET_DB().collection(REPAYMENT_COLLECTION_NAME).findOne({ _id: new ObjectId(String(repaymentId)) }, options)
    return result
  } catch (error) { throw new Error(error) }
}
